 ...
 * 配件管理列表
 */
public class PartsOffLineAdapter extends BaseAdapter<Parts> {

    public PartsOffLineAdapter(Context context) {
        super(context);
    }
    ...
    public View getView(final int i, View view, ViewGroup viewGroup) {
        ViewHolder holder;
        if (view == null) {
            view = inflater.inflate(R.layout.item_parts_off, null);
            holder = new ViewHolder(view);
            view.setTag(holder);
        } else {
            holder = (ViewHolder) view.getTag();
        }
        final Parts bean = data.get(i);
