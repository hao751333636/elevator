...
//        getOnlineRatesPic();
        ...
        mAdapter = new InternetAlarmAdapter(this, list, "状态：", true);
        mListView.setAdapter(mAdapter);
        mListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent intent = new Intent();
                intent.setClass(MonitorActivity.this, MonitorDetailActivity.class);
...
//                    (topBarHeight - llZone.getHeight()) / 2);
    ...
    private void newStatePop() {
        if (popState != null && popState.isShowing()) {
            ...
            SimpleAdapter listAdapter = new SimpleAdapter(
                    this, strStateBeans, R.layout.item_pop_menu,
...
//                    (topBarHeight - llState.getHeight()) / 2);
            popState.showAsDropDown(rlSearch, DisplayUtil.dip2px(this, 8),
                    DisplayUtil.dip2px(this, 17) + llCity.getHeight() + llState.getHeight());
...
//        map.put("IsInstallation", stateId);   //是否安装
        Call<ResponBean> call = server.getService().getProblemList(map);
