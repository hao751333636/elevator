...
import com.sinodom.elevator.adapter.elist.PropertyInspectionDetailsAdapter;
import com.sinodom.elevator.bean.ResponBean;
import com.sinodom.elevator.bean.business.PropertyInspectionDetailsBean;
...
public class PropertyInspectionDetailsActivity extends BaseActivity {
...
//    ScrollView scrollView;
    ...
    private PropertyInspectionDetailsBean recordDetailsBean;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_property_inspection_details);
    ...
    private void initView() {
        if (recordDetailsBean.getPropertyCheckDetails().size() != 0) {
            PropertyInspectionDetailsAdapter adapter = new PropertyInspectionDetailsAdapter(this, recordDetailsBean.getPropertyCheckDetails(), recordDetailsBean.getUser().getUserName());
    ...
    private void getCheck() {
        ...
        Call<ResponBean> call = server.getService().getPropertyCheck(map, id);
            ...
            public void onResponse(Call<ResponBean> call, Response<ResponBean> response) {
                try {
                    if (response.body().isSuccess()) {
                        Gson gson = new Gson();
                        recordDetailsBean = gson.fromJson(response.body().getData(), PropertyInspectionDetailsBean.class);
                        if (recordDetailsBean.getPropertyCheckDetails().size() != 0) {
                            initView();
                        } else {
                            showToast("无应对维保信息");
    ...
    private void setMark(PropertyInspectionDetailsBean bean) {
        double lat = 0;
        double lon = 0;
        try {
