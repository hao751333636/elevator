...
import com.sinodom.elevator.bean.nfc.EquipmentBean;
...
public class EquipmentAddActivity extends BaseActivity {
    ...
    private void loadData() {
        Call<ResponBean> call = server.getService().getLiftEequipment(mLiftNum);
            ...
            public void onResponse(Call<ResponBean> call, Response<ResponBean> response) {
                ...
                try {
                    if (response.body().isSuccess()) {
                        ...
                        EquipmentBean bean = gson.fromJson(response.body().getData(), EquipmentBean.class);
                        tvNfcRegister.setText(bean.getCertificateNum());
                        tvNfcAddress.setText(bean.getAddressPath() + bean.getInstallationAddress());
    ...
    private void bindLift(String certificateNum) {
        showLoading("加载中...");
        Call<ResponBean> call = server.getService().replacEequipment(mLiftNum, certificateNum);
        mRetrofitManager.call(call, new Callback<ResponBean>() {
            @Override
            public void onResponse(Call<ResponBean> call, Response<ResponBean> response) {
                hideLoading();
    ...
    public void location() {
        ...
        if (permission == true && permission1 == true) {
            ...
            ActivityCompat.requestPermissions(EquipmentAddActivity.this,
