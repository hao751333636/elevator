...
import com.sinodom.elevator.adapter.elist.SignAdapter;
...
//        getOnlineRatesPic();
        ...
        mAdapter = new SignAdapter(context, list, "维保结果：", true);
    ...
    private void ShowDialog() {
        ...
        if (popSearch != null && popSearch.isShowing()) {
            ...
            popSearch.showAsDropDown(tvTop, 0,
                    0);
...
//                    (topBarHeight - llCity.getHeight()) / 2);
            popCity.showAsDropDown(tvTop, DisplayUtil.dip2px(context, 8),
...
//                    (topBarHeight - llZone.getHeight()) / 2);
            popZone.showAsDropDown(tvTop, DisplayUtil.dip2px(context, 16) + llCity.getWidth(),
...
//                    (topBarHeight - llState.getHeight()) / 2);
            popState.showAsDropDown(tvTop, DisplayUtil.dip2px(context, 8),
...
//        map.put("IsInstallation", stateId);   //是否安装
        Call<ResponBean> call = server.getService().getUseCheckList(map);
