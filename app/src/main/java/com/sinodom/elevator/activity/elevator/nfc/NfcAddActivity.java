 ...
 * 标签绑定
 */
public class NfcAddActivity extends BaseActivity implements BaseAdapter.OnItemClickListener {
    ...
    private List<NfcBean> mList = new ArrayList<>();
...
//                    loadData();
        ...
        mAdapter = new NfcBindAdapter(context);
            ...
            public void onResponse(Call<ResponBean> call, Response<ResponBean> response) {
                ...
                try {
                    if (response.body().isSuccess()) {
                        ...
                        tvLiftNum.setText("电梯编号：" + bean.getLiftNum());
                        tvAddress.setText("安装地址：" + bean.getAddressPath() + bean.getInstallationAddress());
                    } else {
                    }
                } catch (Exception e) {
                    e.printStackTrace();
    ...
    public void onItemClick(View v, int position) {
        Intent intent = new Intent(NfcAddActivity.this, NfcBindActivity.class);
        intent.putExtra("typeName", mList.get(position).getTermName());
        intent.putExtra("typeContent", mList.get(position).getTermDesc());
        intent.putExtra("LiftId", mList.get(position).getLiftID());
        intent.putExtra("CheckTermId", mList.get(position).getCheckTermId());
        intent.putExtra("CheckTermName", mList.get(position).getTermName());
    ...
    private void bindLift(String certificateNum, double lon, double lat) {
        ...
        Call<ResponBean> call = server.getService().liftSign(map);
        mRetrofitManager.call(call, new Callback<ResponBean>() {
            @Override
            public void onResponse(Call<ResponBean> call, Response<ResponBean> response) {
                hideLoading();
    ...
    public void location() {
        ...
        if (permission == true && permission1 == true) {
            ...
            ActivityCompat.requestPermissions(NfcAddActivity.this,
